...
def plot_accuracy_vs_sigma(acc_dict, save_path=None):
    ...
    ax.set_title('Filter accuracy vs. noise sigma ($\sigma_m$)', fontsize=14)
...
def plot_scalar_vs_sigma(csv_path: str, save_path: str, label: str):
    ...
    ax.set_title(f'{label} vs. noise sigma ($\sigma_m$)', fontsize=14)
    ax.grid(True, linestyle='--', alpha=0.3)
...
def create_combined_figure(figs_dir: str, acc_dict):
    ...
    ax.set_xlabel('Sigma (measurement noise)', fontsize=12)
    ax.set_ylabel('Accuracy (↓)', fontsize=12)
    ax.set_title('Accuracy vs. noise sigma ($\sigma_m$)', fontsize=14)
    ax.grid(True, linestyle='--', alpha=0.3)
    ax.legend(fontsize=9, ncol=2, handlelength=4.5)
    ax.set_xscale('log')
    ...
    if plotted:
        ...
        ax.set_title('PF-CD3Q metrics vs. noise sigma ($\sigma_m$)', fontsize=14)
        ax.grid(True, linestyle='--', alpha=0.3)
        ax.set_xscale('log')
        ax.set_xticks(xticks_all)
